...
import { AlertCircle, Package, ChevronRight, Clock, MessageCircle } from 'lucide-react';
    ...
    <VendorLayout>
      <div className="space-y-8">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold text-slate-800">Litiges clients</h1>
            <p className="text-slate-500 mt-1">Gérez les réclamations reçues</p>
          </div>
        </div>
        ...
        {disputes.length === 0 ? (
          <div className="bg-white rounded-2xl p-12 text-center shadow-sm">
            ...
            <p className="text-slate-500">Aucun litige pour le moment</p>
          </div>
        ) : (
          <div className="space-y-4">
            {disputes.map((dispute) => {
              ...
              return (
                <Link href={`/seller/disputes/${dispute.id}`} key={dispute.id}>
                    ...
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-4 mt-3 text-xs text-slate-400">
                          <span>Client : {dispute.user.name}</span>
                          <span>• Motif : {dispute.reason === 'not_received' ? 'Non reçu' : dispute.reason === 'damaged' ? 'Endommagé' : 'Mauvais produit'}</span>
                        </div>
                      </div>
